...
export default function PlayerAnalysisPage() {
  const { player, playerStats, loading, error, timeWindow } = usePlayerData();
  ...
  const totalWins = playerStats.game.wins.map(w => w.count).reduce((a, b) => a + b, 0);
  ...
  const performanceRating = Math.min(100, Math.round(
    (avgWinPercentage * 0.4) +
    (Math.min(kdRatio * 10, 50) * 0.4) +
    (Math.min(avgKillsPerSeries * 2, 20) * 0.2)
  ));

  const getPerformanceLevel = (rating: number) => {
    if (rating >= 80) return { label: 'Elite', color: 'text-purple-600 dark:text-purple-400' };
    if (rating >= 65) return { label: 'Excellent', color: 'text-blue-600 dark:text-blue-400' };
    if (rating >= 50) return { label: 'Good', color: 'text-green-600 dark:text-green-400' };
    if (rating >= 35) return { label: 'Average', color: 'text-yellow-600 dark:text-yellow-400' };
    return { label: 'Developing', color: 'text-orange-600 dark:text-orange-400' };
  };

  const performance = getPerformanceLevel(performanceRating);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-6 mb-6">
          <div className="flex-shrink-0">
            <div className="w-32 h-32 rounded-full border-8 border-zinc-200 dark:border-zinc-800 flex items-center justify-center">
          ...
          <div className="flex-1">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mb-4">
              Based on win rate, K/D ratio, and average performance metrics
            </p>
      ...
      {/* Recommendations */}
      <div className="p-6 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 rounded-xl border border-blue-200 dark:border-blue-900">
        <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-4 flex items-center gap-2">
